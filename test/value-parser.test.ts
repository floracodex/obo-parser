...
describe('parseSynonym', () => {
  ...
  it('throws on invalid scope', () => {
    expect(() => parseSynonym('"test" INVALID []')).toThrow('Invalid synonym scope');
  });
});
...
describe('parsePropertyValue', () => {
  ...
  it('parses bare ID property value', () => {
    const result = parsePropertyValue('http://purl.org/dc/terms/license http://creativecommons.org/licenses/by/4.0/');
    expect(result).toEqual({
  ...
  it('parses date property value', () => {
    const result = parsePropertyValue('http://purl.org/dc/elements/1.1/date "2026-03-01" xsd:dateTime');
    expect(result).toEqual({
...
describe('parseIsA', () => {
  ...
  it('parses is_a with qualifiers', () => {
    const result = parseIsA('GO:0008150 {gci_relation="part_of", gci_filler="CL:0000000"}');
    expect(result).toEqual({
      target: 'GO:0008150',
      qualifiers: [
...
describe('parseIntersectionOf', () => {
  it('parses bare genus (no predicate)', () => {
    ...
    expect(result).toEqual({ predicate: null, target: 'GO:0007049' });
  });

  it('parses qualified differentia', () => {
    const result = parseIntersectionOf('part_of GO:0008152');
    expect(result).toEqual({ predicate: 'part_of', target: 'GO:0008152' });
  });
});
