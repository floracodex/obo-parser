...
const describeIfFile = (path: string) =>
  existsSync(path) ? describe : describe.skip;

async function collectStanzas(
  input: AsyncGenerator<OboStanza>,
): Promise<OboStanza[]> {
...
describeIfFile(UO_PATH)('UO (Units of Measurement) ontology', () => {
  ...
  it('streaming produces identical results', async () => {
    ...
    expect(streamTerms[streamTerms.length - 1]).toEqual(
      doc.terms[doc.terms.length - 1],
    );
  });
});
