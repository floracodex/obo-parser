...
import type {OboStanza, OboTerm, OboTypedef, OboInstance} from '../src/types.js';
...
describe('parseOboStream', () => {
        ...
        for (const name of fixtureNames) {
            it(`produces equivalent results for ${name}`, async () => {
                ...
                const streamTypedefs = stanzas
                    .filter(
                        (s): s is {type: 'typedef'; typedef: OboTypedef} => s.type === 'typedef'
                    )
                    .map((s) => s.typedef);
                ...
                const streamInstances = stanzas
                    .filter(
                        (s): s is {type: 'instance'; instance: OboInstance} =>
                            s.type === 'instance'
                    )
                    .map((s) => s.instance);
    ...
    describe('edge cases', () => {
        ...
        it('skips empty stanzas (no tags), matching string parser', async () => {
        ...
        it('handles zero-chunk async iterable', async () => {
            async function* noChunks(): AsyncGenerator<string> {
                // yields nothing at all
            }
            const stanzas = await collectStanzas(parseOboStream(noChunks()));
            expect(stanzas).toHaveLength(1);
            expect(stanzas[0].type).toBe('header');
            if (stanzas[0].type === 'header') {
                expect(stanzas[0].header.formatVersion).toBeNull();
            }
        });
        ...
        it('handles input ending without trailing newline', async () => {
            const text = 'format-version: 1.2\n\n[Term]\nid: TEST:0001\nname: root';
            const stanzas = await collectStanzas(parseOboStream(singleChunk(text)));
            expect(stanzas).toHaveLength(2);
            const terms = stanzas.filter((s) => s.type === 'term');
            expect(terms).toHaveLength(1);
            if (terms[0].type === 'term') {
                expect(terms[0].term.name).toBe('root');
            }
        });
        ...
        it('handles ReadableStream input', async () => {
            ...
            const stanzas = await collectStanzas(parseOboStream(stream));
            expect(stanzas).toHaveLength(3);
        });

        it('handles CRLF at end of final chunk (no trailing newline)', async () => {
            const text = 'format-version: 1.2\r\n\r\n[Term]\r\nid: T:1\r\nname: test\r\n';
            const stanzas = await collectStanzas(parseOboStream(chunked(text, 10)));
            const terms = stanzas.filter((s) => s.type === 'term');
            expect(terms).toHaveLength(1);
        });

        it('handles final buffer ending with lone \\r', async () => {
            // Value ends with \r but no \n — the buffer flush path should strip it
            async function* crAtEnd(): AsyncGenerator<string> {
                yield 'format-version: 1.2\n\n[Term]\nid: T:1\nname: test\r';
            }
            const stanzas = await collectStanzas(parseOboStream(crAtEnd()));
            const terms = stanzas.filter((s) => s.type === 'term');
            expect(terms).toHaveLength(1);
            if (terms[0].type === 'term') {
                expect(terms[0].term.name).toBe('test');
            }
        });
    });
});
